...
class ParachuteComponent : ScriptComponent
{
	...
	[Attribute("12.0", UIWidgets.Slider, "Hard Landing Velocity", "0 50 1", category : "Landing")] protected float m_fHardLandingVelocity;

	[Attribute("20.0", UIWidgets.Slider, "Death Landing Velocity", "0 100 1", category : "Landing")] protected float m_fDeathLandingVelocity;

	[Attribute("8.0", UIWidgets.Slider, "Free radius when deploying", "0 100 1", category : "Landing")] protected float m_fSafeRadius;

	[Attribute("6.0", UIWidgets.Slider, "Minimum fallspeed required", "0 100 1", category : "Landing")] protected float m_fMinFallspeed;

	[Attribute("10.0", UIWidgets.Slider, "Minimum altitude to deploy", "0 100 1", category : "Landing")] protected float m_fMinimumAltitude;

	[Attribute("1", UIWidgets.CheckBox, "Break legs", category : "Landing")] protected bool m_bBreakLegs;

	[Attribute("1", UIWidgets.CheckBox, "Death from speed", category : "Landing")] protected bool m_bFallToDeath;

	[RplProp()] bool m_bParachuteDeployed = false;
	...
	void OnControlledEntityChanged(IEntity from, IEntity to)
	{
		...
		invMgr.GetAllRootItems(rootItems);
	...
	[RplRpc(RplChannel.Reliable, RplRcver.Server)] void RpcAskDeployParachute() // ← new parameter
	{
	...
	[RplRpc(RplChannel.Reliable, RplRcver.Owner)] void initializeParachute(vector v0)
	{
	...
	[RplRpc(RplChannel.Reliable, RplRcver.Owner)] void RpcDoSyncReplication(RplId parachuteRplId)
	{
	...
	[RplRpc(RplChannel.Reliable, RplRcver.Server)] void Rpc_ServerBreakLegs()
	{
		if (!m_bBreakLegs)
			return;
		...
		[RplRpc(RplChannel.Reliable, RplRcver.Server)] void Rpc_ServerKillPlayer()
	{
		if (!m_bFallToDeath)
			return;
	...
	[RplRpc(RplChannel.Reliable, RplRcver.Server)] void Rpc_ServerExitParachute(float velocityAtExit)
	{
