...
class ParachuteDeployedEntity: GenericEntity {
	...
	[Attribute("20", UIWidgets.Slider, "Drag Strength", "0 100 1", category: "Flight")]
	...
	protected vector m_WorldTransform[4];
	protected vector m_NwkTargetMatrix[4];
	...
	protected float m_fSumVelocity;
	const protected float m_fDownwardForce = Physics.STANDARD_GRAVITY;
	protected float m_fDownwardVelocity;
	protected float m_ForwardSpeed;
	
	protected float m_Accel;
	protected float m_SmoothAccel;

	protected bool m_HasLanded;
	protected bool m_IsDestroyed = false;


	ref Shape m_DebugShapeWind;
	...
	void DrawDebugUI() {
		...
		DbgUI.Text(string.Format("m_fSumVelocity: %1", m_fSumVelocity));
		DbgUI.Text(string.Format("m_fDownwardForce: %1", m_fDownwardForce));
		DbgUI.Text(string.Format("m_fDownwardVelocity: %1", m_fDownwardVelocity));
		DbgUI.Text(string.Format("m_ForwardSpeed: %1", m_ForwardSpeed));
	...
	bool ShouldSendNetState()
	{
		...
		// 2. Position delta
		vector pos = GetOrigin();    // or m_WorldTransform[3] if already fetched
		if ((pos - m_LastPos).LengthSq() > m_NetPosThreshSq)
			return true;
	
		// 3. Angle delta
		vector ang = Math3D.MatrixToAngles(m_WorldTransform);
	...
	override void EOnFrame(IEntity owner, float timeSlice) {
		...
		vector forwardWorld = VectorToParent(vector.Forward);
	    m_fSumVelocity = vector.Dot(m_vVelocity, forwardWorld.Normalized());
		...
		if (m_RplComponent.IsOwner()) {
			...
			if (m_InputManager.IsContextActive("CharacterMovementContext"))
				m_InputManager.ResetContext("CharacterMovementContext");
	...
	override void EOnContact(IEntity owner, IEntity other, Contact contact)
	{
	    ...
	    if (m_HasLanded || !m_Compartment.IsOccupied())
	        return;
...
override void EOnSimulate(IEntity owner, float timeSlice)
	{
	   ...
	   m_NetAccTime += timeSlice;
		...
		{
			m_NetAccTime = 0;
			if (ShouldSendNetState())     // new gate
			{
				GetWorldTransform(m_WorldTransform);
				Rpc(RpcAsk_NetMovement, m_WorldTransform, m_Physics.GetVelocity(), m_Physics.GetAngularVelocity());
				// record last sent for delta gating
				m_LastPos = m_WorldTransform[3];
				m_LastAngles = Math3D.MatrixToAngles(m_WorldTransform);
			}
		}
	}
	
	void ApplyNetState(vector transform[4], vector vel, vector angVel)
	{
	...
	void RpcAsk_NetMovement(vector transform[4], vector vel, vector angVel)
	{
		// first update authoritative copy on the server
		ApplyNetState(transform, vel, angVel);
	
		// now forward to every proxy except the owner
		Rpc(RpcDo_NetMovement, transform, vel, angVel);
	}
	
	/* Executed on proxies that are NOT the owner */
	[RplRpc(RplChannel.Unreliable, RplRcver.Broadcast, RplCondition.NoOwner)]
	void RpcDo_NetMovement(vector transform[4], vector vel, vector angVel)
	{
		ApplyNetStateProxy(transform, vel, angVel);
	}
	
	void ApplyNetStateProxy(vector target[4], vector vel, vector angVel)
	{
		if (m_RplComponent.IsOwner())
			return;
	...
	{
	    if (!Replication.IsServer() || m_IsDestroyed || !m_DestructableParachute)
	        return;
	...
	void HandleGroundFlare(float timeSlice) {
		...
		m_ForwardSpeed = Math.Max(m_ForwardSpeed - speedLoss, m_MinForwardSpeed);
	
		// 4. Re-apply new horizontal velocity along current nose direction
		vector forwardW = VectorToParent(vector.Forward);
		vector horizVel = forwardW.Normalized() * m_ForwardSpeed;
	...
	void HandleGlide(float timeSlice) {
	    ...
	    float forwardSpeed  = m_ForwardSpeed;
	    float verticalSpeed = m_vVelocity[1];
	
	    // Acceleration from pitch
	    if (normPitch > 0.03)
	        m_Accel =  normPitch * m_GlideDownPitch;
	    else if (Math.AbsFloat(normPitch) <= 0.03)
	        m_Accel =  m_GlideDownPitch * 0.33;
	    else
	        m_Accel =  normPitch * m_GlideUpPitch;
		
	    // Low-pass filter
	    float smoothing     = Math.Clamp(0.5 * timeSlice, 0.0, 0.04);
	    m_SmoothAccel      += (m_Accel - m_SmoothAccel) * smoothing;
		
	    // Integrate speed
	    float prevSpeed     = forwardSpeed;
	    forwardSpeed        = Math.Clamp(
	                            forwardSpeed + m_SmoothAccel * timeSlice,
	...
	void HandleAutoLevel(float timeSlice) {
		...
		vector actualUp = m_WorldTransform[1];  // local Y column (canopy up)
	...
	void HandleDrag(float timeSlice)
	{
		...
		float accel  = (m_fDownwardVelocity - m_MaxFallSpeed) * m_DragStrength; // m s⁻²
		float impulse = accel * m_Physics.GetMass() * timeSlice;                // N s  => kg m s⁻¹
	...
	void PrepareForOccupant(IEntity futureOccupant, ParachuteComponent comp) {
		SCR_ChimeraCharacter m_CharacterOccupant = SCR_ChimeraCharacter.Cast(futureOccupant);
		
		comp.m_CompartmentAccess.GetOnCompartmentLeft().Insert(OnCompartmentLeft);

		Physics occupantPhysics = futureOccupant.GetPhysics();
		vector occupantVelocity = occupantPhysics.GetVelocity();
		vector occupantAngVelocity = occupantPhysics.GetAngularVelocity();

		m_Physics.SetVelocity(occupantVelocity);
		m_Physics.SetAngularVelocity(occupantAngVelocity);
	}
	
	void ApplyWind(float timeSlice) {
		if (!m_weatherManager)
			return;
		...
		if (m_DebugMode) {
			m_DebugShapeWind = DrawDebugLine(GetOrigin(), GetOrigin() + windDirectionVector.AnglesToVector(), COLOR_BLUE);
		}
	...
	void GiveExt(RplIdentity identity, SCR_CompartmentAccessComponent accessComponent) {
		if (!Replication.IsServer())
			return;

		m_RplComponent.GiveExt(identity, true);
	}
	
	bool IsOccupied() {
		return m_Compartment.IsOccupied();
	}
}
